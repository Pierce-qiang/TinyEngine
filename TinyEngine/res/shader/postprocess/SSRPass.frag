...
void main()
{
	...
	vec4 worldPostionFrom = texture(positionTexture, texCoord);
 	...
 	vec3 unitWorldPositionFrom = normalize(worldPostionFrom.xyz - viewPos);
 	vec3 worldNormal           = normalize(texture(normalTexture, texCoord).xyz);
  	vec3 pivot            	   = normalize(reflect(unitWorldPositionFrom, worldNormal));
	...
	endFrag.xy  *= texSize;
	float endViewDistance = (viewMatrix * endPoint).z;
	if(endViewDistance >= 0)
	{
		SSRColor = vec4(0.0);
		return;
	}
    ...
    uv.xy = frag / texSize;
	float viewDistance = startViewDistance;
	...
	float visibility = hit1
		...
		* (1- max(dot(-unitWorldPositionFrom, pivot), 0))
		* (1- clamp( depth / thickness, 0, 1))
		* (1- clamp(length(wolrdPositionTo - worldPostionFrom)/ maxDistance, 0, 1))
		* (uv.x < 0 || uv.x > 1 ? 0 : 1)
		* (uv.y < 0 || uv.y > 1 ? 0 : 1);

	visibility = clamp(visibility, 0, 1);
	SSRColor = vec4(texture(colorTexture, uv.xy).rgb * visibility, 1.0);
	// SSRColor = vec4(uv.xy * visibility,0.0f,1.0f);
	// SSRColor = vec4(abs(pivot), 1.0f);
	// SSRColor = vec4(vec3(abs(deltaX)), 1.0f);
